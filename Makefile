...
all: $(project) ## Generate Protocol Buffer descriptions and build

users: $@ ## Generate Protocol Buffer descriptions and build for users
...
clean: ${RM_F_CMD} ${PROTO_DIR}/*.pb.go ## Clean generated files
	${RM_F_CMD} ssl/*.crt
	${RM_F_CMD} ssl/*.csr
	${RM_F_CMD} ssl/*.key
	${RM_F_CMD} ssl/*.pem
